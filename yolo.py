...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'logs/best_weights-simAM.pth',
        "classes_path"      : 'model_data\\vannamei_classes.txt',
        #---------------------------------------------------------------------#
        #   anchors_path代表先验框对应的txt文件，一般不修改。
        #   anchors_mask用于帮助代码找到对应的先验框，一般不修改。
        #---------------------------------------------------------------------#
        "anchors_path"      : 'model_data\yolo_anchors_alldata.txt',
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            for i in range(thickness):
                ...
                draw.rectangle([(left+right)//2 , (top+bottom)//2, (left+right)//2, (top+bottom)//2], outline=self.colors[c],width=2)
            # draw.rectangle([tuple(text_origin), tuple(text_origin + label_size)], fill=self.colors[c])
            # draw.text(text_origin, str(label,'UTF-8'), fill=(0, 0, 0), font=font)
            # font4count= ImageFont.truetype(font='model_data/simhei.ttf', size=np.floor(1e-2 * image.size[1] + 20).astype('int32'))
            # draw.text([20,20], "number of shrimps: " +str(len(top_label)), fill=(0, 255, 0), font=font4count)
            del draw
            
        return image,len(top_label)
