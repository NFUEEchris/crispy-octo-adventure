...
from nets.backbone import Backbone, Multi_Concat_Block, Conv, SiLU, Transition_Block, autopad,CBAM,SimAM
#cbam now
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        self.sppcspc                = SPPCSPC(transition_channels * 32, transition_channels * 16) #p5
        ...
        #------------------------ CBAM ------------------------# 
        # self.cbam_p3=CBAM(transition_channels * 4,transition_channels * 4)
        # self.cbam_p4=CBAM(transition_channels * 4 , transition_channels * 4)
        # self.cbam_p5=CBAM(transition_channels * 8 , transition_channels * 8)

        self.SimAM_p3=SimAM(transition_channels * 4,transition_channels * 4)
        self.SimAM_p4=SimAM(transition_channels * 4 , transition_channels * 4)
        self.SimAM_p5=SimAM(transition_channels * 8 , transition_channels * 8)
    def fuse(self):
        print('Fusing layers... ')
    ...
    def forward(self, x):
        #  backbone
        feat1, feat2, feat3 = self.backbone.forward(x) #80,40,20
        ...
        P5_conv=self.SimAM_p5(P5_conv) #cbam
        # 20, 20, 256 => 40, 40, 256
        ...
        P4_conv = self.SimAM_p4(P4_conv) #cbam
        # 40, 40, 128 => 80, 80, 128
        P4_upsample = self.upsample(P4_conv)
        # 80, 80, 128 cat 80, 80, 128 => 80, 80, 256
        P3          = torch.cat([self.SimAM_p3(self.conv_for_feat1(feat1)), P4_upsample], 1) #cbam
        # 80, 80, 256 => 80, 80, 128
